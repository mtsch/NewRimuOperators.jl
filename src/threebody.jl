...
diagonal_element(::ThreeBodyMomentumTransfer{<:Any,T}, ::FermiFS, _) where {T} = zero(T)
...
function get_offdiagonal(op::ThreeBodyMomentumTransfer, add_a, add_b, map_a, map_b, i)
...
function diagonal_element(op::ThreeBodyMomentumTransfer, _, _, map_a, map_b)
    value = 0.0
